    ...
    for line in value.split("\n"):
        ...
        if (line.isupper() or line.islower() or (
            ...
            if ignore_nic == True and "nic" in line.lower():
                ...
                if len(words) >= 1:
                    # First word
                    normalized_words.append(normalize_word(words[0], abbreviation_threshold=abbreviation_threshold,
                                                           lowercase_domains=lowercase_domains))
                if len(words) >= 3:
                    # Words between the first and last
                    for word in words[1:-1]:
                        normalized_words.append(normalize_word(word, abbreviation_threshold=abbreviation_threshold,
                                                               lowercase_domains=lowercase_domains,
                                                               never_abbreviations=('as',)))
                if len(words) >= 2:
                    # Last word
                    normalized_words.append(normalize_word(words[-1], abbreviation_threshold=abbreviation_threshold,
                                                           lowercase_domains=lowercase_domains))
